...
size_t LevelSystem::getHeight(){
	return LevelSystem::_height;
}
size_t LevelSystem::getWidth(){
	return LevelSystem::_width;
}
...
sf::Vector2f LevelSystem::getTilePosition(sf::Vector2ul p) {
    ...
    return sf::Vector2f(p.x * _tileSize, p.y * _tileSize) + _offset;
}

std::vector<sf::Vector2ul> LevelSystem::findTiles(TILE tileType) {

    std::vector<sf::Vector2ul> result;

    // Iterate over each tile in the level grid
    for (size_t y = 0; y < _height; ++y) {
        for (size_t x = 0; x < _width; ++x) {
            // Check if the tile at position (x, y) matches the requested type
            if (getTile({ x, y }) == tileType) {
                // If it matches, add the coordinate to the result
                result.push_back({ x, y });
            }
        }
    }
    std::cout << "Player start position: " << result[0] << std::endl;

    // Return the vector of found tile positions
    return result;
}
